import os
import json
import time
import datetime as dt

import pandas as pd
import yfinance as yf
import gspread
from google.oauth2.service_account import Credentials


WS_SYMBOLS = "SYMBOLS"
WS_PRICES = "PRICES"

LOOKBACK_CAL_DAYS = 30
SLEEP_SEC = 0.2


def norm_market(m: str) -> str:
    s = (m or "").strip().lower()
    if s in ("otc", "two", "tpex", "上櫃", "櫃買", "興櫃"):
        return "otc"
    if s in ("tse", "twse", "上市", "tw"):
        return "tse"
    return s


def get_client():
    info = json.loads(os.environ["GOOGLE_SERVICE_ACCOUNT_JSON"])
    scopes = [
        "https://www.googleapis.com/auth/spreadsheets",
        "https://www.googleapis.com/auth/drive",
    ]
    creds = Credentials.from_service_account_info(info, scopes=scopes)
    return gspread.authorize(creds)


def fetch_latest_yf(symbol: str, market: str):
    """
    return (date_str, close, vol_lots)
    vol_lots 一律「張」
    date_str YYYY-MM-DD（交易日）
    """
    market = norm_market(market)
    suffix = ".TW" if market == "tse" else ".TWO"
    ticker = f"{symbol}{suffix}"

    def normalize(df):
        if df is None or df.empty:
            return None

        if isinstance(df.columns, pd.MultiIndex):
            df.columns = [c[0] for c in df.columns]

        if "Close" not in df.columns or "Volume" not in df.columns:
            return None

        last = df.tail(1)
        d = last.index[0].strftime("%Y-%m-%d")
        close = float(last["Close"].values[0])
        vol_shares = float(last["Volume"].values[0])
        vol_lots = int(round(vol_shares / 1000.0))
        return d, close, vol_lots

    end = dt.datetime.utcnow()
    start = end - dt.timedelta(days=LOOKBACK_CAL_DAYS)

    df1 = yf.download(
        ticker,
        start=start.strftime("%Y-%m-%d"),
        end=(end + dt.timedelta(days=1)).strftime("%Y-%m-%d"),
        interval="1d",
        progress=False,
        auto_adjust=False,
        threads=False,
        group_by="column",
    )
    res = normalize(df1)
    if res:
        return res

    df2 = yf.Ticker(ticker).history(period="1mo", interval="1d", auto_adjust=False)
    res = normalize(df2)
    if res:
        return res

    return None


def build_symbol_date_index(ws):
    """
    建立 (symbol, date) -> row_number
    """
    vals = ws.get_all_values()
    if not vals:
        return {}, []

    header = [h.strip().lower() for h in vals[0]]
    rows = vals[1:]

    if "symbol" not in header or "date" not in header:
        raise RuntimeError("PRICES 必須有 Date, Symbol 欄位")

    idx_symbol = header.index("symbol")
    idx_date = header.index("date")

    index = {}
    for row_no, r in enumerate(rows, start=2):
        if len(r) <= max(idx_symbol, idx_date):
            continue
        s = str(r[idx_symbol]).strip()
        d = str(r[idx_date]).strip()
        if s and d:
            index[(s, d)] = row_no

    return index, header


def main():
    sheet_url = os.environ["SHEET_URL"]
    gc = get_client()
    sh = gc.open_by_url(sheet_url)

    # SYMBOLS
    ws_sym = sh.worksheet(WS_SYMBOLS)
    sym_vals = ws_sym.get_all_values()
    if len(sym_vals) < 2:
        raise RuntimeError("SYMBOLS 沒有資料")

    sym_header = [h.strip().lower() for h in sym_vals[0]]
    df = pd.DataFrame(sym_vals[1:], columns=sym_header)

    if "symbol" not in df.columns or "market" not in df.columns:
        raise RuntimeError("SYMBOLS 必須包含 symbol, market")

    if "active" in df.columns:
        df = df[df["active"].astype(str).str.strip() != "0"]

    df["symbol"] = df["symbol"].astype(str).str.strip()
    df["market"] = df["market"].astype(str).map(norm_market)
    df = df[df["market"].isin(["tse", "otc"])]

    if df.empty:
        raise RuntimeError("SYMBOLS 沒有有效股票")

    # PRICES
    ws_p = sh.worksheet(WS_PRICES)
    idx_map, header = build_symbol_date_index(ws_p)

    def col_idx(name):
        name = name.lower()
        if name not in header:
            return None
        return header.index(name) + 1

    c_date = col_idx("date")
    c_symbol = col_idx("symbol")
    c_close = col_idx("close")
    c_vol = col_idx("volume")
    if None in (c_date, c_symbol, c_close, c_vol):
        raise RuntimeError("PRICES 必須有 Date, Symbol, Close, Volume")

    updated = 0
    appended = 0

    for _, r in df.iterrows():
        sym = r["symbol"]
        market = r["market"]

        res = fetch_latest_yf(sym, market)
        if not res:
            print(f"[WARN] {sym}({market}) no data from Yahoo ({sym}{'.TW' if market=='tse' else '.TWO'})")
            time.sleep(SLEEP_SEC)
            continue

        d, close, vol = res
        key = (sym, d)

        if key in idx_map:
            row_no = idx_map[key]
            ws_p.update_cell(row_no, c_close, close)
            ws_p.update_cell(row_no, c_vol, vol)
            updated += 1
            print(f"[UPD] {sym} date={d} row={row_no} close={close} vol(張)={vol}")
        else:
            ws_p.append_row([d, sym, close, vol], value_input_option="USER_ENTERED")
            appended += 1
            idx_map[key] = len(ws_p.get_all_values())
            print(f"[APP] {sym} date={d} close={close} vol(張)={vol}")

        time.sleep(SLEEP_SEC)

    print(f"[DONE] updated={updated}, appended={appended}")


if __name__ == "__main__":
    main()

