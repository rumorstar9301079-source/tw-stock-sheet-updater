...
def fetch_latest_yf(symbol: str, market: str):
    ...
    def normalize(df):
        ...
        vol_lots = int(round(vol_shares / 1000.0))  # ✅ 一律張
...
def build_symbol_index(ws):
    """
    建立 symbol -> row_number 的索引
...
def main():
    ...
    if "symbol" not in df.columns or "market" not in df.columns:
        raise RuntimeError("SYMBOLS 必須包含 symbol, market（market=tse/otc）")
    ...
    # 讀 PRICES 並建立 symbol 索引
    ws_p = sh.worksheet(WS_PRICES)
    sym_index, header = build_symbol_index(ws_p)
    ...
    def col_idx(name):
        ...
        if name not in header:
            return None
        return header.index(name) + 1  # 轉成 1-based column
    ...
    for _, r in df.iterrows():
        ...
        if sym in sym_index:
            ...
            # ✅ 覆蓋同一個 symbol 的那一列（Date/Close/Volume 都更新）
            ws_p.update_cell(row_no, c_date, d)
            ...
            appended += 1
            # 新增後更新 index（避免同一輪再次新增）
            sym_index[sym] = len(ws_p.get_all_values())
